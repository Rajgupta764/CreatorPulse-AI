# Module 10 — Dashboard (Growth Command Center)

> **Goal:** Aggregate data from ALL feature tables (generations, battles, comment_analyses) into a single dashboard response — including trends, streaks, pattern stats, dimension analysis, cross-feature insights, and quick actions.
...
```typescript
// Response DTO — no request needed (GET endpoint)
...
import type { DashboardData, TrendData, ActivityItem, QuickAction, PatternStat, WeeklyAverage, DimensionStat } from "./dto/dashboard.dto";
...
export class DashboardService {
  ...
  async getData(userId: string): Promise<DashboardData> {
    ...
    // Add comment analyses to activity
...
export class DashboardController {
  ...
  @Get()
  async getData(@CurrentUser() user: { id: string }) {
...
|---|---|---|---|
| GET | `/api/dashboard` | Required | Aggregated dashboard data |
...
```json
{
  "totalGenerations": 47,
  "totalBattles": 12,
  "totalCommentAnalyses": 5,
  "averageViralityScore": 68,
  "bestViralityScore": 92,
  "streak": 6,
  "lastActiveDate": "2026-07-22T...",
  "scoreTrend": { "direction": "up", "percentChange": 22, "slope": 3.5 },
  "weeklyAverages": [
    { "week": "2026-06-29", "avgScore": 58 },
    { "week": "2026-07-06", "avgScore": 72 }
  ],
  "mostUsedPattern": "Curiosity Gap",
  "bestPerformingPattern": "How-To",
  "patternStats": [
    { "pattern": "Curiosity Gap", "count": 15, "avgScore": 82 }
  ],
  "weakestDimension": { "name": "emotion", "avgScore": 43 },
  "strongestDimension": { "name": "curiosity", "avgScore": 78 },
  "crossFeatureInsights": ["Your most used pattern is Curiosity Gap."],
  "recentActivity": [
    { "type": "generation", "title": "How to edit faster", "score": 78, "date": "...", "link": "/generate" }
  ],
  "quickActions": [
    { "label": "Analyze a Title", "description": "Check a title's viral potential", "link": "/generate", "priority": "high" }
  ]
...
### Files to DELETE
```
src/app/api/dashboard/route.ts  → replaced by NestJS controller
src/lib/dashboard-service.ts     → moved to NestJS DashboardService
```

### Files to UPDATE
```
src/app/dashboard/page.tsx  → fetch /api/dashboard with JWT token
```
...
- [ ] `GET /api/dashboard` returns all fields in the DashboardData shape
- [ ] Requires JWT authentication (returns 401 without token)
- [ ] Empty state works for new users with no data
- [ ] Streak computes consecutive active days correctly
- [ ] Score trend direction (up/down/flat) is accurate
- [ ] Weekly averages show last 8 weeks
- [ ] Pattern stats are sorted by count descending
- [ ] Cross-feature insights reference actual data patterns
- [ ] Quick actions adapt based on user activity level
