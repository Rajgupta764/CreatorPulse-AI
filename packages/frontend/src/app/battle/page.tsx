  ...
  return (
    <>
      <form onSubmit={handleSubmit} className="space-y-4 rounded-xl border border-border bg-card p-6">
        <input value={titleA} onChange={(e) => setTitleA(e.target.value)} placeholder="Title A" required className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
        <input value={titleB} onChange={(e) => setTitleB(e.target.value)} placeholder="Title B" required className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" />
      ...
      {result && (
        <div className="mt-8 space-y-4">
          ...
          {result.hybridTitle && (
            <div className="rounded-xl border border-border bg-card p-6">
              <p className="text-xs uppercase tracking-wide text-muted-foreground">Hybrid Suggestion</p>
  ...
  return (
    <ToolPageLayout
      ...
      steps={[
        ...
        { num: 3, title: "Get the fusion", desc: "See the winner and a hybrid title combining the strongest parts of each." },
      ]}
    >
